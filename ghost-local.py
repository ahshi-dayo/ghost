...
チャットコマンド:
  /recall              記憶を再想起
  /search <query>      記憶を検索
  /delusion <query>    完全記憶検索（忘却・バイアスなし）
  /delusion --date D   指定日の全記憶ダンプ
  /overview            脳の俯瞰
...
def handle_command(cmd, model, messages):
    ...
    if command == "/recall":
    ...
    elif command == "/delusion":
        if not arg:
            print("使い方: /delusion <検索語>  or  /delusion --date 2024-12-11")
        else:
            # argをそのままmemory.py delusionに渡す
            delusion_args = ["delusion"] + arg.split()
            print(run_memory(*delusion_args, timeout=60))
        return model, True

    elif command == "/overview":
        print(run_memory("overview", timeout=60))
        return model, True
        ...
        print("コマンド: /recall /search /delusion /overview /sleep /mood /stats /model /help")
        return model, True
